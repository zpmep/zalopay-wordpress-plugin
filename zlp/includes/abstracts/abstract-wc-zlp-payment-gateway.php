 ...
 * @since 4.0.0
 */
abstract class WC_ZaloPay_Payment_Gateway extends WC_Payment_Gateway {
	/**
	...
	public function validate_minimum_order_amount( $order ) {
		if ( $order->get_total() * 100 < WC_Stripe_Helper::get_minimum_amount() ) {
			...
			throw new WC_ZaloPay_Exception( 'Did not meet minimum amount', sprintf( __( 'Sorry, the minimum allowed order total is %1$s to use this payment method.', 'woocommerce-gateway-stripe' ), wc_price( WC_Stripe_Helper::get_minimum_amount() / 100 ) ) );
		}
	}
	...
	public function get_source_object( $source_id = '' ) {
		...
		if ( ! empty( $source_object->error ) ) {
			throw new WC_ZaloPay_Exception( print_r( $source_object, true ), $source_object->error->message );
		}
		...
		if ( ! empty( $_POST['stripe_source'] ) ) {
			...
			if ( ( $user_id && $this->saved_cards && $maybe_saved_card && 'reusable' === $source_object->usage ) || $force_save_source ) {
				...
				if ( ! empty( $response->error ) ) {
					throw new WC_ZaloPay_Exception( print_r( $response, true ), $this->get_localized_error_message_from_response( $response ) );
				}
			}
			...
			if ( ! $wc_token || $wc_token->get_user_id() !== get_current_user_id() ) {
				...
				throw new WC_ZaloPay_Exception( 'Invalid payment method', __( 'Invalid payment method. Please input a new card number.', 'woocommerce-gateway-stripe' ) );
			}
			...
			if ( ( $user_id && $this->saved_cards && $maybe_saved_card ) || $force_save_source ) {
				...
				if ( ! empty( $response->error ) ) {
					throw new WC_ZaloPay_Exception( print_r( $response, true ), $response->error->message );
				}
				$source_id    = $response;
			} else {
	...
	public function update_fees( $order, $balance_transaction_id ) {
		...
		if ( empty( $balance_transaction->error ) ) {
			...
			WC_ZaloPay_Logger::log( 'Unable to update fees/net meta for order: ' . $order->get_id() );
		}
	}
	...
	public function add_payment_method() {
		...
		if ( $error ) {
			...
			WC_ZaloPay_Logger::log( 'Add payment method Error: ' . $error_msg );
			return;
		}
	...
	public function create_intent( $order, $prepared_source ) {
		...
		$order_id = $order->get_id();
		WC_ZaloPay_Logger::log( "Stripe PaymentIntent $intent->id initiated for order $order_id" );
	...
	public function confirm_intent( $intent, $order, $prepared_source ) {
		...
		if ( 'succeeded' === $confirmed_intent->status ) {
			WC_ZaloPay_Logger::log( "Stripe PaymentIntent $intent->id succeeded for order $order_id" );
		} elseif ( 'requires_action' === $confirmed_intent->status ) {
			WC_ZaloPay_Logger::log( "Stripe PaymentIntent $intent->id requires authentication for order $order_id" );
		}
	...
	private function get_intent( $intent_type, $intent_id ) {
		...
		if ( $response && isset( $response->{ 'error' } ) ) {
			...
			WC_ZaloPay_Logger::log("Failed to get Stripe intent $intent_type/$intent_id.");
			WC_ZaloPay_Logger::log("Response: $error_response_message");
			return false;
		}
	...
	public function setup_intent( $order, $prepared_source ) {
		...
		if ( is_wp_error( $setup_intent ) ) {
			WC_ZaloPay_Logger::log( "Unable to create SetupIntent for Order #$order_id: " . print_r( $setup_intent, true ) );
	...
	public function create_and_confirm_intent_for_off_session( $order, $prepared_source, $amount = NULL ) {
		...
		$order_id       = $order->get_id();
		WC_ZaloPay_Logger::log( "Stripe PaymentIntent $intent_id initiated for order $order_id" );
